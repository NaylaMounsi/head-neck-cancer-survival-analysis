...
-- Déduplication des patients : creation d'une base sans doublons --
...
FROM patients ;
-- > On récupère seulement 17 obs , on a 400 données manquantes sur 521 obs : la variable combinée créée pour maximiser les obs disponibles n'est pa ssuffisante, l'information concernant le hpv n'est pas exploitable dans le modèle de survie multivarié car baisse de l'échantillon --
...
-- delcarer les valeur vide en nulles --
SET SQL_safe_UPDATES = 0;
UPDATE tumor
...
group by lymphvascular_invasion,perineural_invasion;
 -- > 71 patho_stage null, 14 clinical_stage null, 158 perineural_invasion_null, 172 lymphvascular_invasion_null : valeur manquante inherente a la methode de traitement et informations manquante pour les patients qui n'ont pas été opérés problème de biais de selection majeur, qui justifie un modéle independant sur les individus opéré (exclusion des variables pour le modele principal) --
